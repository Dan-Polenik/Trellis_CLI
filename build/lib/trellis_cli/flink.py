...
services:
  ...
  beam_job_server:
    image: apache/beam_flink1.18_job_server:2.66.0
    container_name: trellis-beam-flink-js
    command: >
      --flink-master=jobmanager:8081 --job-host=0.0.0.0 --job-port=8099 --artifact-port=8098 --expansion-port=8097
    depends_on: [ jobmanager, taskmanager ]
    ports: [ "8099:8099", "8098:8098", "8097:8097" ]
    volumes:
    - "{str(BEAM_JAR_DIR)}:/opt/beam/jars:ro"
    environment:
  ...
  go_harness:
    ...
    command: >
      --worker_pool --id=go-external --port=0.0.0.0:50000
    ports: [ "50000:50000" ]
    depends_on: [ beam_job_server ]
